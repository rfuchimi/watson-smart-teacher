...
const DISCOVERY_DOCS = [
  './data/discovery/docs/documento01.docx',
  './data/discovery/docs/documento02.docx'
]
      ...
      function (err, tone) {
        ...
        if (payload.input.text != '') {
          ...
          nlu.analyze(parameters, function (err, response) {
            if (err) {
              //console.log("acessou o erro")
              console.log('error:', err)
            } else {

              //console.log("acessou o else do NLU")
    ...
    if (data.context.action.lookup === LOOKUP_BALANCE) {
      ...
      if (data.context.action.account_type && data.context.action.account_type != '') {
        ...
        bankingServices.getAccountInfo(7829706, data.context.action.account_type, function (err, accounts) {
          ...
          const appendAccountResponse = !!(data.context.action.append_response && data.context.action.append_response === true)
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (accounts[i].available_credit) { accounts[i].available_credit = accounts[i].available_credit ? numeral(accounts[i].available_credit).format('INR 0,0.00') : '' }

            if (accounts[i].last_statement_balance) { accounts[i].last_statement_balance = accounts[i].last_statement_balance ? numeral(accounts[i].last_statement_balance).format('INR 0,0.00') : '' }
      ...
      bankingServices.getBranchInfo(loc, function (err, branchMaster) {
        ...
        const appendBranchResponse = !!(data.context.action.append_response && data.context.action.append_response === true)
      ...
      if (!discoveryParams) {
        ...
        discovery.query(queryParams, (err, searchResponse) => {
          discoveryResponse = 'Sinto muito. Não consegui obter as respostas neste momento. Entraremos em contato para ajudá-lo!'
          if (err) {
